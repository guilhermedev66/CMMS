...
public sealed class IdentityAccessDesignTimeDbContextFactory
    : IDesignTimeDbContextFactory<IdentityAccessDbContext>
{
    public IdentityAccessDbContext CreateDbContext(string[] args)
    {
        ...
        var options = new DbContextOptionsBuilder<IdentityAccessDbContext>()
            .UseNpgsql(
                connectionString,
                postgres => postgres.MigrationsHistoryTable("__ef_migrations_history", DatabaseSchemas.IdentityAccess))
