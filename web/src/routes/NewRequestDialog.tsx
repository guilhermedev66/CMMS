...
/** Lightweight intake form per docs/04-frontend-ia.md: "asset or location if asset unknown, description, priority". */
export function NewRequestDialog({ onClose, onCreated }: NewRequestDialogProps) {
  const { user } = useAuth()
  const sites = user?.siteMemberships ?? []
  ...
  const [targetKind, setTargetKind] = useState<'asset' | 'location'>('asset')
  ...
  const target = targetKind === 'asset' ? assetId : locationId
  const canSubmit = title.trim().length > 0 && siteId && target && !submitting
  ...
  async function handleSubmit() {
    ...
    try {
      await createRequest({
        ...
        assetId: targetKind === 'asset' ? assetId : null,
        locationId: targetKind === 'location' ? locationId : null,
        priority,
      })
      onCreated()
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div role="dialog" aria-modal="true" aria-label="New Request" className="w-full max-w-md rounded-md border border-border bg-surface-raised p-4">
        <h2 className="mb-3 text-sm font-semibold text-text-primary">New Request</h2>

        {sites.length === 0 ? (
          <p className="text-sm text-text-secondary">
            You have no site membership, so there's nowhere to submit a request against yet.
          </p>
        ) : (
          <div className="flex flex-col gap-3">
            ...
            <Field label="Description">
              <textarea
                ...
                rows={3}
                placeholder="What's wrong, and any details that help triage…"
            ...
            <div className="flex gap-2">
              <label className="flex items-center gap-1.5 text-sm text-text-primary">
                <input type="radio" checked={targetKind === 'asset'} onChange={() => setTargetKind('asset')} />
                Asset
              </label>
              <label className="flex items-center gap-1.5 text-sm text-text-primary">
                <input type="radio" checked={targetKind === 'location'} onChange={() => setTargetKind('location')} />
                Location (asset unknown)
              </label>
            </div>
